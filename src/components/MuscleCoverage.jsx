  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-slate-900">Muscle Coverage</h2>
        <div className="flex rounded-lg bg-slate-100 p-1">
          {[7, 30].map((d) => (
            <button
              key={d}
              className={`rounded-md px-3 py-1 text-sm font-medium ${days === d ? 'bg-white text-brand-700 shadow-sm' : 'text-slate-500'}`}
              onClick={() => setDays(d)}
            >
              {d}d
            </button>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="card space-y-2">
            <h3 className="font-semibold text-slate-900">Breakdown (last {days} days)</h3>
            {data.coverage.map((c) => (
              <div key={c.muscle} className="flex items-center gap-2">
                ...
                />
                <span className="w-24 shrink-0 text-sm capitalize text-slate-700">{c.muscle}</span>
            ...
            <p className="pt-1 text-xs text-slate-400">
              ...
              exercises in the window. Blue = under-worked relative to an even split, red = over-worked.
            </p>
          </div>
        </>
      )}
